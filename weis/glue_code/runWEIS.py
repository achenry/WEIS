...
def run_weis(fname_wt_input, fname_modeling_options, fname_opt_options, overridden_values=None):
    ...
    # Initialize openmdao problem. If running with multiple processors in MPI, use parallel finite differencing equal
    # to the number of cores used. Otherwise, initialize the WindPark system normally. Get the rank number for
    # parallelization. We only print output files using the root processor.
    ...
    if MPI:
        ...
        # Define the color map for the parallelization, determining the maximum number of parallel finite difference
        # (FD) evaluations based on the number of design variables (DV). OpenFAST on/off changes things.
        if modeling_options['Level3']['flag']:

            # If we are running an optimization method that doesn't use finite differencing, set the number of DVs to 1
            if not (opt_options['driver']['design_of_experiments']['flag'] or opt_options['driver']['optimization'][
                'solver'] in fd_methods):
                n_DV = 1

            # If openfast is called, the maximum number of FD is the number of DV, if we have the number of cores
            # available that doubles the number of DVs, otherwise it is half of the number of DV (rounded to the
            # lower integer). We need this because a top layer of cores calls a bottom set of cores where OpenFAST
            # runs.
            if max_cores > 2. * n_DV:
                n_FD = n_DV
            else:
                n_FD = int(np.floor(max_cores / 2))
            # Get the number of OpenFAST runs from the user input and the max that can run in parallel given the
            # resources The number of OpenFAST runs is the minimum between the actual number of requested OpenFAST
            # simulations, and the number of cores available (minus the number of DV, which sit and wait for OF to
            # complete)
    ...
    if color_i == 0:  # the top layer of cores enters, the others sit and wait to run openfast simulations
        ...
        if MPI:
            ...
            if opt_options['driver']['design_of_experiments']['flag']:
                ...
                wt_opt.model.add_subsystem('comp', WindPark(modeling_options=modeling_options, opt_options=opt_options),
                                           promotes=['*'])
        else:
